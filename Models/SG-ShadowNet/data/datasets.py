...
class ImageDataset(Dataset):
    ...
    def __init__(self, root, unaligned=False, mode='train'):
        ...
        #print(os.path.join(root, 'test_A'))
        self.files_A = sorted(glob.glob(os.path.join(root, 'train_A') + '/*.*'))
        self.files_B = sorted(glob.glob(os.path.join(root, 'train_C') + '/*.*'))
        self.files_mask = sorted(glob.glob(os.path.join(root, 'train_B') + '/*.*'))
        self.files_mask50 = sorted(glob.glob(os.path.join(root, 'train_B') + '/*.*'))
...
class TestImageDataset(Dataset):
    def __init__(self, root, mode='test'):
        self.files_A = sorted(glob.glob(os.path.join(root, 'test_A') + '/*.*'))                # full shadow image
        self.files_B = sorted(glob.glob(os.path.join(root, 'test_C') + '/*.*'))
        self.files_mask = sorted(glob.glob(os.path.join(root, 'test_B') + '/*.*'))
