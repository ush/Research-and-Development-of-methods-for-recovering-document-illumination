...
def get_parser():
    ...
    parser.add_argument('-om', '--output_maps', type=bool, default=False)
    parser.add_argument('-dt', '--dataset_type', type=str)
...
def test_own_image(G1, G2, path, out_path, size, img_transform):
    ...
    with torch.no_grad():
        ...
        #комменты проставил Стас, убрать если что начиная с (этой) до (этой + 10) строчки.
...
def main(parser):
    ...
    if parser.load is not None:
        ...
        G1_weights = torch.load('./checkpoints/'+parser.dataset_type+'/ST-CGAN_G1_'+parser.load+'.pth')
        G1.load_state_dict(fix_model_state_dict(G1_weights))

        G2_weights = torch.load('./checkpoints/'+parser.dataset_type+'/ST-CGAN_G2_'+parser.load+'.pth')
