...
def get_parser():
    ...
    parser.add_argument('-e', '--epoch', type=int, default=1500, help='Number of epochs')
    ...
    parser.add_argument('-dp', '--data_path', type=str, default='../../Dataset/train/')
    parser.add_argument('-dt', '--data_type', type=str, default='classic')
...
def evaluate(G1, G2, dataset, device, filename):
    img, gt_shadow, gt = zip(*[dataset[i] for i in range(4)])
    img = torch.stack(img)
...
def train_model(G1, G2, D1, D2, dataloader, val_dataset, num_epochs, parser, save_model_name='model'):
    ...
    for epoch in range(num_epochs+1):
        ...
        if(epoch%50 == 0):
            torch.save(G1.state_dict(), 'checkpoints/'+parser.data_type+'/'+save_model_name+'_G1_'+str(epoch)+'.pth')
            torch.save(G2.state_dict(), 'checkpoints/'+parser.data_type+'/'+save_model_name+'_G2_'+str(epoch)+'.pth')
            torch.save(D1.state_dict(), 'checkpoints/'+parser.data_type+'/'+save_model_name+'_D1_'+str(epoch)+'.pth')
            torch.save(D2.state_dict(), 'checkpoints/'+parser.data_type+'/'+save_model_name+'_D2_'+str(epoch)+'.pth')
            G1.eval()
            G2.eval()
...
def main(parser):
    ...
    print("loading data!")
    train_img_list, val_img_list = make_datapath_list(phase='train', rate=parser.hold_out_ratio, path_to_data = parser.data_path)
